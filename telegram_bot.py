...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "<b>Что это?</b>\n"
        "AI-советчик по трейдингу. Анализирует акции по 8 факторам и выдаёт торговый план с entry, stop, targets.\n\n"
        "<b>Как читать сигнал?</b>\n"
        "• <b>Score</b> — сила (-1…+1). Чем выше |score|, тем сильнее.\n"
        "• <b>Tier</b> — класс: <b>A</b> (сильный), <b>B</b> (средний), <b>C</b> (слабый/нет)\n"
        "• <b>Conf</b> — уверенность модели (0–100%). Показывает согласованность факторов:\n"
        "  Conf ≥80% — факторы согласованы; Conf 50–70% — есть расхождения; Conf &lt;50% — слабо.\n"
        "• <b>R:R</b> — соотношение риска/прибыли (всегда ≥ 1.5)\n\n"
        "<b>Что такое Conf?</b>\n"
        "Conf — НЕ вероятность прибыли, а согласованность компонентов (техника, импульс, новости, объём). "
        "Считается по: согласованности знаков (45%), средней силе (35%), диверсификации (20%) + бонус до +18%. "
        "Высокий Conf → больше размер позиции, низкий → меньше.\n\n"
        "<b>🧠 Kronos FM</b> — PyTorch-трансформер, прогнозирует цену по истории свечей. "
        "Работает только в <b>/signal</b> (не в /screen). Вклад: адаптивный вес 0.05–0.25.\n\n"
        "<b>Команды бота:</b>\n"
        "<b>🔍 Анализ:</b> /signal тикер — полный анализ; /price — быстрая цена; /pick — подбор\n"
        "<b>📊 Скринер:</b> /screen — все рынки; /screen_ru — РФ; /screen_us — US; /dashboard — свод\n"
        "<b>📁 Списки:</b> /watchlist — управление; /notify — уведомления по списку\n"
        "<b>🤖 Сбор:</b> /collect — сбор сигналов; /export — выгрузка; /status — статус сбора\n"
        "<b>🧠 ML:</b> /learning — обучение; /stats — статистика; /backtest — бэктест; /force_learn (админ)\n"
        "<b>💎 Premium:</b> /clusters /mlscore /portfolio /alerts /alerts_add /alerts_remove\n"
        "<b>⚙️ Настройки:</b> /settings — фильтры, уведомления, автосбор, дайджест, просадки\n"
        "<b>👑 Админ:</b> /approve /deny /revoke /users\n"
        "<b>📍 Навигация:</b> /start /menu /cancel /help\n\n"
        "<b>🌍 Рынки:</b> US (Polygon+Finnhub+Yahoo) | RU (Т-Банк+MOEX+Yahoo)\n\n"
        "<b>⚠️ Важно:</b> Это информационный инструмент, НЕ финансовая рекомендация. Торговля сопряжена с риском."
    )
    uid = _uid(update)
