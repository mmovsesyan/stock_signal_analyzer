...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if _is_approved(uid):
        text = (
            ...
            "<b>🔍 Анализ</b>\n"
            "Техника • Импульс • Новости • Объём • Макро\n\n"
            "<b>📋 Торговый план</b>\n"
            "Стоп • Цели • R:R • Chandelier • Размер позиции\n\n"
            "<b>🏷️ Классы сигналов</b>\n"
            "A — сильный  •  B — средний  •  C — наблюдение\n\n"
            "<b>🌍 Рынки</b>\n"
            "US (Polygon, Finnhub)  •  РФ (T-Bank, MOEX)\n\n"
            "<b>🧠 AI Learning</b>\n"
    ...
    welcome_text = (
        ...
        "<b>🧠 AI Learning</b>\n"
        "Система совершенствуется на исходах сделок\n\n"
        "Отправьте тикер — получите полный разбор."
    )
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "• <b>Conf</b> — уверенность модели (0–100%). Показывает согласованность факторов.\n"
        "• <b>R:R</b> — соотношение риска к прибыли (всегда ≥ 1.5)\n\n"
        "<b>Что такое Conf (уверенность)?</b>\n"
        "Conf — это НЕ вероятность прибыли. Это внутренняя метрика, которая показывает,\n"
        "насколько согласованы между собой компоненты анализа (техника, импульс, новости, объём).\n\n"
        "<b>Как считается Conf:</b>\n"
        "• <b>Согласованность знаков (45%)</b> — все ли факторы смотрят в одну сторону.\n"
        "  Если 4 из 4 в плюс — максимум. Если 2 в плюс, 2 в минус — Conf падает.\n"
        "• <b>Средняя сила (35%)</b> — насколько сильны сами значения факторов.\n"
        "• <b>Диверсификация (20%)</b> — штраф, если один фактор доминирует в 3+ раза.\n"
        "  Хороший сигнал подтверждается разными факторами, а не одним.\n"
        "• <b>Бонус (+10…+18%)</b> — если все факторы одного знака и сильные (&gt;0.15).\n\n"
        "<b>Как интерпретировать Conf:</b>\n"
        "• <b>Conf ≥ 80%</b> — компоненты сильно согласованы, сигнал многофакторный.\n"
        "• <b>Conf 50–70%</b> — есть расхождения, возможно 1–2 фактора противоречат.\n"
        "• <b>Conf &lt; 50%</b> — слабая согласованность, сигнал на 1–2 факторах.\n\n"
        ...
        "<b>Влияние Conf на торговлю:</b>\n"
        "• Conf влияет на размер позиции: высокий Conf = больше позиция, низкий = меньше.\n"
        "• При Conf &lt; 30% позиция минимальна (5%).\n"
        "• При Conf &gt; 60% позиция может достигать 100% от базового размера.\n"
        "• Conf также используется ML-моделью для оценки качества сигнала.\n\n"
        "<b>📋 Команды бота:</b>\n\n"
        "<b>🔍 Анализ и котировки</b>\n"
        "• /signal &lt;тикер&gt; — полный анализ акции с торговым планом (AAPL, SBER.ME).\n"
        "  Показывает Score, Tier, Conf, уровни входа, стопа, целей.\n"
        "• /price &lt;тикер&gt; или /quote — быстрая цена без анализа.\n"
        "• /pick &lt;фильтр&gt; — подбор тикеров по критериям (например, AAPL,GAZP.ME).\n\n"
        "<b>📊 Скринер и рынок</b>\n"
        "• /screen — скринер всех рынков (US + RU), выводит топ-сигналы A/B/C.\n"
        "• /screen_ru — только российские акции (.ME).\n"
        "• /screen_us — только американские акции.\n"
        "• /dashboard — свод по вашему watchlist с текущими сигналами.\n\n"
        "<b>📁 Списки и отслеживание</b>\n"
        "• /watchlist — управление списком отслеживания.\n"
        "  Можно добавить свои тикеры или использовать дефолтные (голубые фишки).\n"
        "• /notify — отправить уведомление по всем тикерам из watchlist.\n\n"
        "<b>🤖 Сбор и экспорт данных</b>\n"
        "• /collect — ручной сбор сигналов по всей вселенной тикеров.\n"
        "• /export — выгрузить лог сигналов в JSONL для бэктеста.\n"
        "• /status или /статус — статус последнего сбора (сколько обработано).\n\n"
        "<b>🧠 Обучение и статистика</b>\n"
        "• /learning — отчёт обучения ML-модели по исходам сделок (win/loss/timeout).\n"
        "• /stats — статистика исходов: win rate, avg PnL, распределение по типам выхода.\n"
        "• /backtest — запуск бэктеста на исторических данных.\n"
        "  Проверяет, как сигналы работали в прошлом.\n"
        "• /force_learn — принудительный запуск обучения модели (админ).\n\n"
        "<b>💎 Premium-фичи (требуют подписки)</b>\n"
        "• /clusters — кластерный анализ акций (группировка по корреляции).\n"
        "• /mlscore — ML-оценка качества сигнала на основе исторических исходов.\n"
        "• /portfolio — управление портфелем и отслеживание позиций.\n"
        "• /alerts — настройка алертов (ценовые триггеры, сигналы).\n"
        "• /alerts_add &lt;тикер&gt; &lt;цена&gt; — добавить ценовой алерт.\n"
        "• /alerts_remove &lt;id&gt; — удалить алерт.\n\n"
        "<b>⚙️ Настройки (/settings)</b>\n"
        "Интерактивное меню с переключателями:\n"
        "• <b>Фильтр сигналов</b> — conservative (только A, строго), "
        "balanced (A+B), aggressive (все сигналы).\n"
        "• <b>Уведомления вне watchlist</b> — сигналы по всему рынку, "
        "а не только из списка (с выбором рынка: 🌍 Все / 🇺🇸 Только US / 🇷🇺 Только РФ).\n"
        "• <b>Learning report</b> — ежедневный отчёт об обучении модели.\n"
        "• <b>Автосбор</b> — автоматический сбор сигналов каждые N секунд.\n"
        "• <b>Просадки</b> — уведомления при падении акций из watchlist.\n"
        "• <b>Дайджест</b> — ежедневный дайджест сигналов (morning/evening).\n"
        "• <b>Дефолтные тикеры</b> — использовать стандартный список голубых фишек.\n\n"
        "<b>📍 Навигация</b>\n"
        "• /start или /menu — главное меню с Reply-клавиатурой.\n"
        "• /cancel — отменить текущее действие.\n"
        "• /help — эта справка.\n\n"
        "<b>👑 Админ-команды</b>\n"
        "• /approve &lt;id&gt; — одобрить пользователя.\n"
        "• /deny &lt;id&gt; — заблокировать пользователя.\n"
        "• /revoke &lt;id&gt; — отозвать доступ.\n"
        "• /users — список всех пользователей.\n\n"
        "<b>🌍 Рынки:</b>\n"
        "US — Polygon + Finnhub + Yahoo Finance\n"
        "RU — Т-Банк (Мосбиржа) + MOEX ISS + Yahoo fallback\n\n"
        "<b>⚠️ Важно:</b>\n"
...
def main() -> int:
    ...
    app.add_handler(CallbackQueryHandler(_on_admin_action, pattern=r"^adm\|(approve|deny)"))
