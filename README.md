...
- 🧠 **Kronos Foundation Model** — PyTorch-трансформер для прогноза цены по свечам (только `/signal`, не скринер)
...
## 📖 Как работает анализ (для новичков)
...
1. **Загружает историю** — котировки за 12+ месяцев.
2. **Считает 8 факторов** — каждый дает оценку от −1 (медвежий) до +1 (бычий).
3. **Объединяет** — итоговый score (−1…+1).
4. **Строит торговый план** — entry, stop, target 1, target 2, размер позиции.
5. **Проверяет макро** — ближайшие важные события (Fed, CPI, и т.д.).

### 8 факторов (что каждый означает)
...
**Компоненты подробно**
Каждый фактор расписан отдельно:
...
**🧠 Kronos FM**
- Показывается только в `/signal` (не в `/screen`).
- **5d pred: +3.46%** — модель предсказывает рост на 3.46% за 5 дней.
...
### Самообучение (почему система умнеет)
Каждые 6 часов:
1. Система смотрит на прошлые сигналы — какие сработали (win_t1, win_t2), какие нет (loss).
2. Считает **IC (Information Coefficient)** для каждого фактора — насколько он предсказывает результат.
3. **LLM (Ollama/gemma3:4b)** анализирует топ-5 wins и losses, ищет паттерны.
4. Корректирует веса: факторы с положительным IC получают больше веса, с отрицательным — меньше.
...
### Режим рынка (bull / bear / sideways)
