...
### Fixed
- **`adaptive_weights.py`**: компоненты с отрицательным IC больше не получают boost через `abs(ic_val)`
  - Раньше: IC = −0.10 и IC = +0.10 получали одинаковый boost → система усиливала "вредные" компоненты
  - Теперь: положительный IC → boost, отрицательный IC → penalty (минимум 30% от базового веса)
  - Предотвращает сходимость adaptive learning к обратно-коррелированным предикторам
