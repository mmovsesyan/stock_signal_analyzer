...
3. Рыночные и макро-условия, в которых стратегия работает лучше/хуже
4. Оптимальные пороги качества сигнала

ВАЖНО: корректировки весов факторов применяются автоматически на основе числового анализа. НЕ дублируй их в текстовых советах. Рекомендации должны быть про рыночный контекст, макро и паттерны, а не про «увеличить вес X на Y%».
...
Правила:
- weight_recommendations: вычисли на основе статистики, но программа применит их автоматически — тебе не нужно повторять это текстом
- recommendations: только качественные аналитические инсайты (макро, сектора, волатильность, корреляции), НЕ про веса факторов
